...
Интерфейс на русском — для English-speaking рецензентов: правый клик на странице →
«Translate to English» (родной переводчик Chrome/Edge). На мобильном браузерный
перевод местами всё ещё может ломать вёрстку — известное ограничение, не баг
этого README.

Бэкенд + фронтенд для отслеживания производства POSM (подставки, корнеры, дисплеи)
от ТЗ бренда до доставки в торговые точки (ТТ). Задачи проходят **10 этапов**
...
`render.yaml` описывает все три сервиса одним Blueprint: API (Docker), managed
Postgres и фронт как статический сайт (Render сам проксирует `/api/*` на API-сервис
через `routes`, так что nginx для облака не нужен — он остаётся только для
Docker Compose ниже). Жми кнопку, залогинься в Render через GitHub — через пару
минут получишь публичный URL.

Особенности бесплатного тарифа:
- API-сервис засыпает после 15 мин простоя — первый запрос после этого грузится
  ~30–50 сек.
- Бесплатная Postgres-БД имеет ограниченный срок жизни (см. текущие условия на
  render.com/pricing) — по истечении нужно будет пересоздать БД (данные — это
  демо-сид, ничего не жалко) или перейти на платный инстанс.
- По умолчанию `UPLOAD_DIR` указывает на `/tmp` — файлы не переживают рестарт
  контейнера. Задай `R2_ACCOUNT_ID` / `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` /
  `R2_BUCKET_NAME` (Cloudflare R2, S3-совместимо, бесплатно до 10 ГБ) в Render
  Dashboard — при всех четырёх заданных загрузки идут туда вместо локального диска.
- AI-ассистент в облаке не работает (ему нужен locally-running Ollama) — по
  умолчанию выключен (`LLM_ENABLED=false`) и корректно показывает это в UI.
...
| `PATCH /api/tasks/{code}`           | manager, admin    | обновить (этап, сроки, поля ТЗ)  |
