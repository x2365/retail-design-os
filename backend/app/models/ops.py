...
class Delivery(Base, TimestampMixin):
    ...
    retail_point_id: Mapped[int] = mapped_column(ForeignKey("retail_points.id", ondelete="RESTRICT"), index=True)
    status: Mapped[DeliveryStatus] = mapped_column(Enum(DeliveryStatus), default=DeliveryStatus.pending)
    region: Mapped[ShipmentRegion] = mapped_column(Enum(ShipmentRegion), default=ShipmentRegion.local, index=True)
...
class Document(Base, TimestampMixin):
    ...
    stage: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)  # 1..12 (tab it belongs to)
