...
def stage_history_to_out(h: models.TaskStageHistory) -> schemas.StageHistoryOut:
    ...
    return schemas.StageHistoryOut(
        id=h.id,
        from_stage=h.from_stage, from_stage_label=lbl(h.from_stage),
        to_stage=h.to_stage, to_stage_label=lbl(h.to_stage),
...
def task_to_out(task: models.Task, counts: dict | None = None) -> schemas.TaskOut:
    ...
    return schemas.TaskOut(
        code=task.code, name=task.name, brand=task.brand.name, group=task.brand.group.code,
        stage=task.stage, urgent=task.urgent, currency=task.currency,
        created_at=(task.created_at.isoformat() if task.created_at else None),
        production_cost=task.production_cost,
        budget=task.budget, sample_cost=task.sample_cost, tirazh_cost=task.tirazh_cost,
        prepaid=task.prepaid,
        tt_total=c["tt_total"], tt_ok=c["tt_ok"], tt_partial=c["tt_partial"], tt_miss=c["tt_miss"],
        days_left=task.days_left, progress_pct=task.progress_pct, stage_name=task.stage_name,
        stage_code=task.stage_code,
        deadline=_fmt_date(task.deadline_tt), launch=_fmt_date(task.launch_date),
        team=[m.name for m in task.members],
        team_detail=[{"name": m.name, "email": m.email or "", "telegram": m.telegram or ""} for m in task.members],
...
def approval_to_out(a: models.Approval) -> dict:
    return {
        "id": a.id, "from_name": a.from_name, "role": a.role, "summary": a.summary,
        "type": a.type, "avatar": a.avatar, "color": a.color, "status": a.status.value,
        "comment": a.comment,
...
def payment_to_out(p: models.Payment) -> dict:
    ...
    PS_LABELS = {"registry": "Отправлен в реестр", "queued": "Заведён на оплату",
                 "prepaid": "Предоплачен", "paid": "Оплачено"}
    if ps in PS_LABELS:
        status = PS_LABELS[ps]
    ...
    return {
        "id": t.code, "brand": t.brand.name, "name": t.name,
...
def task_to_tt(task: models.Task, counts: dict) -> dict:
    return {
        "task": task.code, "brand": task.brand.name, "name": task.name,
        "tt_total": counts["tt_total"], "tt_ok": counts["tt_ok"],
        "tt_partial": counts["tt_partial"], "tt_miss": counts["tt_miss"],
...
def delivery_to_out(d: models.Delivery) -> dict:
    ...
    return {
        "id": d.id, "task": d.task.code,
        "point_code": d.retail_point.code, "point_name": d.retail_point.name,
        "city": d.retail_point.city, "status": d.status.value,
...
def nomenclature_to_out(n: models.NomenclatureItem) -> dict:
    ...
    return {
        "id": n.id, "task": n.task.code, "sku": n.sku, "barcode": n.barcode,
        "name": n.name, "qty": n.qty, "status": n.status.value,
