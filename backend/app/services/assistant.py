...
def tool_list_tasks(db: Session, brand: str | None = None, group: str | None = None,
                    stage: int | None = None, urgent: bool | None = None,
                    overdue: bool | None = None, limit: int = 50) -> dict:
...
def tool_get_task(db: Session, code: str) -> dict:
    ...
    d["gates"] = {
        ...
        "kp": {"manager": bool(t.kp_manager_approved_at), "director": bool(t.kp_director_approved_at),
               "network": bool(t.kp_network_approved_at)},
        "sample": {"qc": bool(t.sample_qc_approved_at), "brand": bool(t.sample_brand_approved_at),
...
def tool_get_kpis(db: Session) -> dict:
    active = db.scalar(select(func.count()).select_from(models.Task).where(models.Task.stage < 12)) or 0
    closed = db.scalar(select(func.count()).select_from(models.Task).where(models.Task.stage >= 12)) or 0
...
def tool_group_budgets(db: Session) -> dict:
    ...
    return {"groups": [{
        ...
        "planned": _money(g.budget_planned), "spent": _money(g.budget_spent),
    } for g in rows]}
...
def tool_pending_approvals(db: Session) -> dict:
    ...
    return {"count": len(items), "approvals": [
        {"task": a["task"], "type": a["type"], "who": a["role"], "summary": a["summary"]} for a in items
    ]}
...
def tool_list_deliveries(db: Session, code: str) -> dict:
    ...
    for d in rows:
        p = d.retail_point
        res.append({"point": p.name if p else None, "city": p.city if p else None,
                    "status": ru.get(d.status.value, d.status.value)})
    return {"count": len(res), "deliveries": res}


def tool_list_contractors(db: Session) -> dict:
    rows = db.scalars(select(models.Contractor).where(models.Contractor.is_active == True)).all()  # noqa: E712
...
TOOLS: dict[str, tuple[dict, Callable]] = {
    ...
    "get_task": ({"type": "object", "properties": {"code": {"type": "string"}}, "required": ["code"]}, tool_get_task),
    ...
    "list_deliveries": ({"type": "object", "properties": {"code": {"type": "string"}}, "required": ["code"]}, tool_list_deliveries),
...
def _llm_chat(messages: list[dict], tools: list[dict] | None) -> dict:
    ...
    payload: dict[str, Any] = {"model": settings.llm_model, "messages": messages, "temperature": 0.2}
    if tools:
        payload["tools"] = tools
...
def run_assistant(db: Session, query: str, screen: str | None = None,
    ...
    try:
        ...
        return {"answer": (data["choices"][0]["message"].get("content") or "").strip(), "used_tools": used}
    except httpx.ConnectError:
        return {"error": "no_connection",
                "answer": "Не удалось подключиться к локальной модели. Запущена ли Ollama (localhost:11434)?"}
    except httpx.HTTPStatusError as e:
        return {"error": "llm_http",
                "answer": f"Модель вернула ошибку: {e.response.status_code}. Проверьте, что модель «{settings.llm_model}» загружена (ollama pull)."}
