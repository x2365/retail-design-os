...
def _add(db: Session, *, user_id, task_id, stage, rule, message, dedup_key) -> bool:
    ...
    exists = db.scalar(select(func.count()).select_from(models.Notification)
                       .where(models.Notification.dedup_key == dedup_key))
    if exists:
        return False
    db.add(models.Notification(
        user_id=user_id, task_id=task_id, stage=stage, rule=rule,
        message=message, dedup_key=dedup_key, status="pending",
    ))
    return True
...
def build_due_notifications(db: Session, now: dt.datetime | None = None) -> int:
    ...
    open_tasks = list(db.scalars(
        select(models.Task).where(models.Task.stage < models.LAST_STAGE)
    ).all())
    ...
    for t in open_tasks:
        ...
        if age_days > SLA_STAGE_STUCK_DAYS and recipients:
            for u in recipients:
                msg = f"Задача {t.code}: этап «{label}» без движения {age_days} дн. (SLA {SLA_STAGE_STUCK_DAYS})."
                if _add(db, user_id=u.id, task_id=t.id, stage=t.stage, rule="stage_stuck",
                        message=msg, dedup_key=f"stage_stuck:{t.id}:{t.stage}:{u.id}:{day}"):
                    created += 1
        ...
        if dl and recipients:
            ...
            if days_left <= SLA_DEADLINE_DAYS:
                when = (f"через {days_left} дн." if days_left >= 0 else f"просрочен на {-days_left} дн.")
                for u in recipients:
                    msg = f"Задача {t.code}: дедлайн ТТ {dl.date().isoformat()} — {when}"
                    if _add(db, user_id=u.id, task_id=t.id, stage=t.stage, rule="deadline",
                            message=msg, dedup_key=f"deadline:{t.id}:{u.id}:{day}"):
                        created += 1

    # 3) согласования висят (pending дольше порога) → менеджерам
    pend = list(db.scalars(
    ...
    for a in pend:
        ...
        if age > SLA_APPROVAL_PENDING_DAYS and managers:
            ...
            for u in managers:
                ...
                if _add(db, user_id=u.id, task_id=(a.task.id if a.task else None), stage=None,
                        rule="approval_pending", message=msg,
...
def dispatch_pending(db: Session, dry_run: bool | None = None) -> dict:
    ...
    rows = list(db.scalars(
        select(models.Notification).where(models.Notification.status == "pending")
    ).all())
    sent = failed = 0
    for n in rows:
        channels: list[str] = []
